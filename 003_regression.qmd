...
Submit the application assignment [here](https://canvas.wisc.edu/courses/345486/assignments/1827995) and complete the [unit quiz](https://canvas.wisc.edu/courses/345486/quizzes/424945) by 8 pm on Wednesday, February 7th
...
```{r u3-packages, messages = FALSE, warning = FALSE}
...
- Lets glimpse the feature matrix to see that it looks as expected
  - Sample size is correct
  - It contains outcome and our one feature
```{r}
feat_trn |> glimpse()
```

- Alternatively, we could skim it.  This might be better than glimpse because it provides more detail
  - Sample size correct
  - It containes outcome and our one feature
...
- the formula indicating the features in our model

```{r u3-simple-5}
fit_lm_1 <- 
  linear_reg() |> # <1>
  set_engine("lm") |> # <2> 
  fit(sale_price ~ gr_liv_area, data = feat_trn) # <3>
```
1. set the broad catetory of algorithm
2. set the specific engine - a general linear model
3. indicate the model formula (just dv and features) and use training set features as data
...
feat_val |> 
  ggplot(aes(x = gr_liv_area)) +
    ...
    geom_line(aes(y = predict(fit_lm_1, data_val)$.pred), linewidth = 1.25, color = "blue") + 
...
```
1.  notice that we now include four predictors in the recipe to get features for all of these predictors 
...
fit_lm_4 <- 
  ...
  fit(sale_price ~ ., # <1>
      data = feat_trn)
```
1. Here we demonstrate the use of `.` to indicate the use of all columns in the feature matrix (other than the outcome) as predictors.  This is a useful shorthand for formulas in `fit()` and `recipe()`.
