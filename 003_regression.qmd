...
* [Lecture 7: Distance and Normalization in KNN](https://www.loom.com/share/eaa39314f59d437e85b6a5e49c2a2959); ~8 mins
...
Submit the application assignment [here](https://canvas.wisc.edu/courses/345486/assignments/1827995) and complete the [unit quiz](https://canvas.wisc.edu/courses/345486/quizzes/424945) by 5 pm on Wednesday, February 8th
...
```{r u3-overview-1, messages = FALSE, warning = FALSE}
# set up packages
...
```{r}
library(conflicted)
tidymodels_prefer() # resolve common function conflicts
...
data_trn <- read_csv(here(path_data, "ames_clean_class_trn.csv"), 
                      col_types = list(
                        sale_price = col_double(),
                        gr_liv_area = col_double(),
                        lot_area = col_double(),
                        year_built = col_double(),
                        overall_qual = col_factor(levels = as.character(1:10), 
                                                  ordered = TRUE),
                        garage_cars = col_double(),
                        garage_qual = col_factor(
                          levels = c("no_garage", "po", "fa", "ta", "gd", "ex"),
                          ordered = TRUE),
                        ms_zoning = col_factor(),
...
```{r u3-overview-3}
data_val <- read_csv(here(path_data, "ames_clean_class_val.csv"),
                     col_types = list(
                        sale_price = col_double(),
                        gr_liv_area = col_double(),
                        lot_area = col_double(),
...
Pause for a moment to answer this question: `r colorize("Why do we need independent validation data to select the best model configuration?  In other words, why cant we just fit and evaluate all of the models in our one training set?", "red")`
...
* Currently, we prefer to include all predictors in the formula at this point
* We will reduce down to just relevant predictors for our feature matrix later
```{r u3-simple-1}
rec <- 
...
* Remember we wrote a function in the last unit to do this together
...
```{r,  u3-simple-4, message = FALSE, warning = FALSE}
...
library(discrim) # needed for these engines
show_engines("discrim_linear") # https://www.tidyverse.org/blog/2019/10/discrim-0-0-1/
show_engines("discrim_regularized")  # for range of LDA to QDA in a regularized solution
...
* The updated formula to include only our first predictor in raw form (no additional feature engineering on the predictor)
...
fit_lm_1 <- 
  linear_reg() %>% 
  set_engine("lm") %>% 
  fit(sale_price ~ gr_liv_area, data = feat_trn)
```
...
Option 2: Extract a single estimate using $ and row number.  Be careful that order of features won't change!
```{r u3-simple-8}
...
Best to write a function if we plan to do this a lot
```{r u3-simple-10}
# included in fun_modeling.R
...
* We prep the recipe with `data_trn` (though not important here because our recipe doesn't do anything yet!)
* We bake `data_val`
...
We can visualize how well this model performs in the validation set by plotting `sale_price` (**ground truth** in machine learning terminology) vs. $\hat{sale\_price}$ for those data

We might do this a lot so let's write a function
...
error_val <- bind_rows(error_val, 
                        tibble(model = "simple linear model", 
                               rmse_val = rmse_vec(truth = feat_val$sale_price, estimate = predict(fit_lm_1, feat_val)$.pred)))
...
ggplot(aes(x = gr_liv_area)) +
  ...
  geom_line(aes(y = predict(fit_lm_1, data_val)$.pred), size = 1.25, color = "blue") + 
...
Remember from the last unit that we have missing data on `garage_cars` in the training set

```{r u3-simple-20}
sum(is.na(data_trn$garage_cars))
```
...
Now we need to prep the recipe in training and bake both the training and validation sets
...
*** 
Now let's combine our algorithm and training features to fit this model configuration

```{r u3-simple-23}
fit_lm_4 <- 
  linear_reg() %>% 
  set_engine("lm") %>% 
  fit(sale_price ~ gr_liv_area + lot_area + year_built + garage_cars, data = feat_trn)
```
...
Let's look at correlations in the training set.  But we need to select down to just these features

```{r u3-simple-27}
feat_trn %>% 
  select(sale_price, gr_liv_area, lot_area, year_built, garage_cars) %>% 
  cor() %>% 
  corrplot.mixed()
```

`r colorize("What are the implications of the correlations among many of these predictors?", "red")`
...
plot_grid(plot_1, plot_4, labels = list("1 feature", "4 features"), hjust = -1.5)
```
...
```{r u3-simple-29}
rmse_vec(feat_val$sale_price, predict(fit_lm_1, feat_val)$.pred)
```

* The four feature linear model.  A clear improvement!
```{r u3-simple-30}
rmse_vec(feat_val$sale_price, predict(fit_lm_4, feat_val)$.pred)
```
...
error_val <- 
  bind_rows(error_val, 
            tibble(model = "4 predictor linear model", 
...
error_val <- bind_rows(error_val, 
                        tibble(model = "4 predictor linear model with YJ", 
                               rmse_val = rmse_vec(feat_val$sale_price, predict(fit_lm_4yj, feat_val)$.pred)))

error_val
```

That didn't help at all.   We may need to consider a transformation of `sale_price` or a different algorithm that can handle non-linear relationships better.
...
data_dummy <- data_trn %>% 
  ...
  mutate(ms_zoning3 = case_when(ms_zoning == "rh" ~ "residential",
                               ms_zoning == "rm" ~ "residential",
                               ms_zoning == "rl" ~ "residential",
                               ms_zoning == "a" ~ "commercial",
                               ms_zoning == "c" ~ "commercial",
                               ms_zoning == "i" ~ "commercial",
                               ms_zoning == "fv" ~ "floating")) %>% 
  select(-ms_zoning)
...
We need to convert this three-level, unordered categorical predictor to numeric features to allow us to use it in statistical algorithms like the general linear model.


`r colorize("Why can't we simply recode each level with a different consecutive value (e.g., residential = 1, comercial =2 , floating = 3)?", "red")`
...
plot_3d <- d %>% 
  scatterplot3d(angle=55, label.tick.marks = FALSE)
...
* Combine all the commercial categories (a, c, i), which would take care of most of the low n groups.  They also all tend to have the lower prices.
*  Combine all the residential (rh, rl, rm) to get a better feature to variance accounted ratio.  They all tend to have similar prices on average and rh is also pretty low n. 
...
* duplex: Duplex
* twhs_ext:	Townhouse End Unit
* twhs_int:	Townhouse Inside Unit
...
* We will collapse categories down to three levels (commercial, residential, floating village) as before but now using `step_mutate()` combined with `fct_collapse()` because it is already a factor
...
* `step_novel()` to handle unanticipated levels that are found in val or test but not train.
...
* Remember to use selector functions.  For example, you could make dummy variables out of all of your factors using `step_dummy(all_nominal_predictors())`.  
...
* Notice the addition of the dummy coded features to the feature matrix (including a feature for the "new" level)
* Notice the removal of the factor `ms_zoning`
* Notice the use of the names for dummy coded features in the fit formula
```{r u3-categorical-6}
feat_trn <- rec %>% 
  make_features(data_trn, data_trn)
...
fit_lm_41cat <- 
  linear_reg() %>% 
  set_engine("lm") %>% 
  fit(sale_price ~ gr_liv_area + lot_area + year_built + garage_cars + ms_zoning_floating + ms_zoning_commercial, data = feat_trn)

plot_truth(truth = feat_val$sale_price, 
           estimate = predict(fit_lm_41cat, feat_val)$.pred)

error_val <- error_val %>% 
  bind_rows(tibble(model = "4 predictor linear model and ms_zoning", 
                   rmse_val = rmse_vec(feat_val$sale_price,
                                       predict(fit_lm_41cat,
                                               feat_val)$.pred)))
...
`r colorize("Will the addition of new predictors/features to a model always reduce RMSE in train?  in validation?", "red")`
...
rec <- 
  ...
  step_mutate(ms_zoning = fct_collapse(ms_zoning,
                                       ...
                                       floating = "fv")) %>%
  step_dummy(ms_zoning) %>% 
  step_ordinalscore(overall_qual)
...
fit_lm_5_1cat <- 
  linear_reg() %>% 
  set_engine("lm") %>% 
  fit(sale_price ~ gr_liv_area + lot_area + year_built + garage_cars + overall_qual, data = feat_trn)


plot_truth(truth = feat_val$sale_price, 
                                 estimate = predict(fit_lm_5_1cat, feat_val)$.pred)

error_val <- bind_rows(error_val, 
                        tibble(model = "5 predictor linear model + ms_zoning", 
                               rmse_val = rmse_vec(feat_val$sale_price, predict(fit_lm_5_1cat, feat_val)$.pred)))
...
fit_lm_5_1cat_int <- 
  linear_reg() %>% 
  set_engine("lm") %>% 
  fit(sale_price ~ gr_liv_area + lot_area + year_built + garage_cars + overall_qual + overall_qual_x_year_built
      , data = feat_trn)


plot_truth(truth = feat_val$sale_price, 
           estimate = predict(fit_lm_5_1cat_int, feat_val)$.pred)

error_val <- bind_rows(error_val, 
                      tibble(model = 
                               "5 predictor linear model + ms_zoning + interaction", 
                             rmse_val = rmse_vec(feat_val$sale_price,
                                                 predict(fit_lm_5_1cat_int,
                                                         feat_val)$.pred)))
...
* Can use "~ .^2" to include all two way interactions (be careful if you have dummy code features!)
...
Notice we are removing `lot_config` and `bldg_type as well with `step_rm()`.  This will allow us to use the `.` operator in `fit()`
```{r u3-interactions-6}
rec <- 
  recipe(sale_price ~ ., data = data_trn) %>% 
  step_impute_median(garage_cars) %>% 
  ...
  step_rm(lot_config, bldg_type, garage_qual)
...
Notice that the feature matrix now only contains the features we care about

We can now use `.` to select all features in the feature matrix
...
plot_truth(truth = feat_val$sale_price, 
           estimate = predict(fit_lm_5_1int, feat_val)$.pred)

error_val <- error_val %>% 
  bind_rows(tibble(model = "5 predictor linear model + ms_zoning & its interaction", 
                   rmse_val = rmse_vec(feat_val$sale_price,
                                       predict(fit_lm_5_1int,
                                               feat_val)$.pred)))
...
* Is a non-parametric regression and classification statistical algorithm
  ...
  * There are still ways to use it to address explanatory questions (visualizations, model comparisons)
...
```{r u3-knn-1, echo = FALSE}
set.seed(55231)
trn_demo <- tibble(x = runif(150, 1, 100),
               y = rnorm(150, x, 10),
               y_dgp = rnorm(150, x ,0),
...
`r colorize("What would 5-NN predictions look like for each of these three new values of X?", "red")`
...
```{r u3-knn-2, echo = FALSE}
trn_demo %>% 
  ggplot(aes(x = x, y = y)) +
    geom_line(aes(x = x, y = y_dgp, color = "blue"), size = 1.5) +
...
```{r u3-knn-7}
rec <- 
  recipe(y2 ~ ., data = trn_demo)

trn_demo_baked <- rec %>% 
  make_features(trn_demo, trn_demo)
```
...
```{r u3-knn-9}
val_demo <- tibble(x = runif(200, 1, 100),
               y = rnorm(200, x, 10),
               y_dgp = rnorm(200, x ,0),
...
val_demo_baked <- rec %>% 
  make_features(trn_demo, val_demo)
```
...
val_demo_baked %>% 
  ggplot(aes(x = x, y = y2)) +
    geom_line(aes(x = x, y = y2_dgp, color = "blue"), size = 1.5) +
    geom_smooth(aes(color = "green"), method = 'lm',formula = y ~ x,  se = FALSE) +
    geom_line(aes(x = x, y = predict(fit_demo_5nn, val_demo_baked)$.pred, color = "red"), size = 1.5) +
    geom_point(aes(y = y2), color = "black") +
    scale_color_identity(name = "Model",
...
`r colorize("How will the size of k influence model performance (e.g., bias, overfitting/variance)?", "red")`
...
`r colorize("How will k = 1 perform in training and validation sets?", "red")`
...
```{r u3-knn-14, echo = FALSE}
plot_train <- trn_demo_baked %>% 
  ggplot(aes(x = x, y = y2)) +
    geom_line(aes(x = x, y = y2_dgp, color = "blue"), size = 1.5) +
    geom_line(aes(x = x, y = predict(fit_demo_5nn, trn_demo_baked)$.pred, color = "red"), size = 1.5) +
    geom_line(aes(x = x, y = predict(fit_demo_1nn, trn_demo_baked)$.pred, color = "green"), size = 1.5) +
    geom_point(aes(y = y2), color = "black") +
    scale_color_identity(name = "Model",
...
plot_val <- val_demo_baked %>% 
  ggplot(aes(x = x, y = y2)) +
    geom_line(aes(x = x, y = y2_dgp, color = "blue"), size = 1.5) +
    geom_line(aes(x = x, y = predict(fit_demo_5nn, val_demo_baked)$.pred, color = "red"), size = 1.5) +
    geom_line(aes(x = x, y = predict(fit_demo_1nn, val_demo_baked)$.pred, color = "green"), size = 1.5) +
    geom_point(aes(y = y2), color = "black") +
    scale_color_identity(name = "Model",
                       breaks = c("blue", "red", "green"),
                       labels = c("DGP", "k = 5", "k = 1"),
...
`k = 1`
```{r u3-knn-15}
rmse_vec(val_demo_baked$y2, 
         predict(fit_demo_1nn, val_demo_baked)$.pred)
```

`k = 5`
```{r u3-knn-16}
rmse_vec(val_demo_baked$y2, 
         predict(fit_demo_5nn, val_demo_baked)$.pred)
```
...
```{r u3-knn-19, echo = FALSE}
plot_train <- trn_demo_baked %>% 
  ggplot(aes(x = x, y = y2)) +
    geom_line(aes(x = x, y = y2_dgp, color = "blue"), size = 1.5) +
    geom_line(aes(x = x, y = predict(fit_demo_5nn, trn_demo_baked)$.pred, color = "red"), size = 1.5) +
    geom_line(aes(x = x, y = predict(fit_demo_1nn, trn_demo_baked)$.pred, color = "green"), size = 1.5) +
    geom_line(aes(x = x, y = predict(fit_demo_75nn, trn_demo_baked)$.pred, color = "yellow"), size = 1.5) +
    geom_point(aes(y = y2), color = "black") +
    scale_color_identity(name = "Model",
...
plot_val <- val_demo_baked %>% 
  ggplot(aes(x = x, y = y2)) +
    ...
    geom_line(aes(x = x, y = predict(fit_demo_5nn, val_demo_baked)$.pred, color = "red"), size = 1.5) +
    geom_line(aes(x = x, y = predict(fit_demo_1nn, val_demo_baked)$.pred, color = "green"), size = 1.5) +
    geom_line(aes(x = x, y = predict(fit_demo_75nn, val_demo_baked)$.pred, color = "yellow"), size = 1.5) +
    geom_point(aes(y = y2), color = "black") +
     scale_color_identity(name = "Model",
                       breaks = c("blue", "red", "green", "yellow"),
                       labels = c("DGP", "k = 5", "k = 1", "k = 75"),
...
```{r u3-knn-22}
rmse_vec(val_demo_baked$y2, 
         predict(fit_demo_75nn, val_demo_baked)$.pred)
```
***
...
distance2 %>% 
  ggplot(aes(x = x1, y = x2)) +
    ...
    geom_line(aes(x = x1h, y = x2h), color = "gray", size = 1.5) +
    geom_line(aes(x = x1v, y = x2v), color = "gray", size = 1.5) +
...
distance2 %>% 
  ggplot(aes(x = x1, y = x2)) +
    geom_point(color = c("red", "blue"), size = 3) +
    geom_line(aes(x = x1h, y = x2h), color = "green", size = 1.5) +
    geom_line(aes(x = x1v, y = x2v), color = "green", size = 1.5) +
    geom_text(x = .9, y = 2, label = "p", size = 5) +
    geom_text(x = 2.1, y = 5, label = "q", size = 5) +
    geom_text(x = 1.5, y = 1.8, label = "A = 1", size = 5) +
    geom_text(x = 2.2, y = 3.5, label = "B = 3", size = 5) +
...
Distance is dependent on scales of all the features

* We need to put all features on the same scale

  * Typical to range correct [0, 1] all features
  * Or to scale all features to SD = 1

* KNN generally accommodates categorical variables as dummy variables (like linear models)
  * Dummy coding will be the same in recipe as before
...
* Train a model using only numeric predictors and $overall\_qual$ as numeric
...
fit_5nn_5num <- 
  ...
  set_mode("regression") %>% 
  fit(sale_price ~ 
        gr_liv_area + lot_area + year_built + garage_cars + overall_qual, 
      data = feat_trn)
```
...
Overall, we now have a model that predicts housing prices with about 35K of error and accounting for 84% of the variance.  I am sure you can improve on this!
