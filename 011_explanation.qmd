...
explain_full <- explain_tidymodels(fit_full, # our model object 
                                   ...
                                   y = y, # outcome
                                   predict_function = predict_wrapper, # our predictor function
                                   predict_function_target_column = ".pred_yes") # the name of the column for predictions
```
...
- Each subsequent row shows the accuracy of the model when that specific feature is permuted
...
imp_permute_group <- model_parts(explain_full, 
                               ...
                               loss_function = accuracy_wrapper,
                               B = 100,
                               variable_groups = list(exer_test = c("exer_ang_yes",
                                                                    "exer_max_hr",
                                                        "exer_st_depress", 
...
#       mutate(shaps = map(dfs, \(df1) get_shaps(df1))) |> view()
#       select(-dfs) |>
#       unnest(shapleys) 
#   },
#   rerun = rerun_setting,
