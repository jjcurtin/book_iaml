...
Basic setup
```{r u9-overview-1, warning=FALSE, message=FALSE}
library(iml) # NEW: for feature importance, plots, etc
library(tidyposterior) # NEW: for perf_mod()
...
Handle additional function conflicts
```{r conflicts}
library(conflicted)
...
Permutation tests can be used to determine if a specific model is performing better than would be expected by chance.

However, we often want to compare two models against each other.

Model comparisons can also be used for explanatory goals
...
We will learn approaches to statistically compare models
...
We will compare model configurations formed by feature ablation but nothing would be different if these model configurations different by other characteristics such as statistical algorithm
...
* Max heart hr during the exercise test
* experience of angina during the test
* The slope of the peak exercise ST segment (don't ask me what that is!  ;-)
...
```{r u9-compare-1}
data_all <- read_csv(file.path(path_data, "cleveland.csv"), col_names = FALSE, na = "?",col_types = cols()) %>% 
  rename(age = X1,
         sex = X2,
         cp = X3,
         rest_bp = X4,
...
Lets take a quick look at a feature matrix using all the data to make sure things are as we expect

```{r u9-compare-3}
rec_full

feat_all_full <- rec_full %>% 
...
```{r u9-compare-6}
fits_full %>% 
  plot_hyperparameters(hp1 = "penalty", hp2 = "mixture", metric = "accuracy", log_hp1 = TRUE)

exp(-8)
```

`r colorize("Does the glmnet (regularized logistic regresssion) outperform a simple logistic regression?  How would these two algorithms compare?", "red")` ^[Remember that the linear model is a special case of glmnet where the penalty = 0.  Here we are looking log penalty down to -8, or `r exp(-8)`.  That is pretty close to 0 so an approximation of how the standard logistic regression would perform.  glmnet is about 2% more accurate with its optimal hyperparameter values]

***

Here are the optimal hyperparameter values
```{r u9-compare-7}
...
fit_full <- 
  ...
  set_engine("glmnet") %>% 
  fit(disease ~ ., data = feat_all_full)
```

We don't care too much about objective performance in our situation.  That is not our question.

* Same sample accuracy is comparable to what you would normally have if you weren't in a machine learning context
* Bootstrapped accuracy is better because it is prediction in new data vs. same sample
* K-fold would be less biased than bootstrap if we cared about performance of the overall model
* Some optimization bias using either k-fold or bootstrapped accuracy to both select hyperparameters AND assess performance of best model
* If we really cared about overall performance, we should have held back a test set (or done nested CV)

```{r u9-compare-9}
accuracy_vec(feat_all_full$disease, predict(fit_full, feat_all_full)$.pred_class)
...
Our focal question is NOT how well our full model does.  It is how much do scores from our exercise test improve prediction above baseline (with baseline being all other available characteristics).  

We need to use **feature ablation** to form a compact/reduced model that does not include the features associated with our exercise test

Here is a recipe to feature engineer features associated with this compact model

Note: `step_rm(contains("exer_")`

All else is the same


```{r u9-compare-10}
rec_compact <- recipe(disease ~ ., data = data_all) %>% 
  step_rm(contains("exer_")) %>%                    # remove key feature(s)
  step_impute_median(all_numeric_predictors()) %>% 
  step_impute_mode(all_nominal_predictors()) %>%   
  step_dummy(all_nominal_predictors()) %>% 
  step_normalize(all_predictors())
...
We need to select/tune hyperparameters for this new model.  It has different complexity that the full model so might need different regularization for optimal performance.
...
fit_compact <- 
  ...
  set_engine("glmnet") %>% 
  fit(disease ~ ., data = feat_all_compact)
```
...
```{r u9-compare-14}
accuracy_vec(feat_all_full$disease, predict(fit_full, feat_all_full)$.pred_class)
accuracy_vec(feat_all_compact$disease, predict(fit_compact, feat_all_compact)$.pred_class)
```
...
It is less accurate but.....

* A simple descriptive comparison is not sufficient to justify the use of a costly test

* We need to be more confident that the test really improves screening in all possible held out samples.
...
* We have used the same 100 held-out samples (we used the same splits) so these two sets of accuracies (for each of the two models) should be considered paired/repeated.   
  * Not a problem, we could use paired samples t-test
  * Easiest to thing about this paired test as testing if the differences in accuracy for each of the 100 held out sets == 0.  That removes the lack of independence from using the sample 100 held-out sets twice
...
It is adjusted by $\frac{\rho}{1-\rho}$ where $\rho = \frac{n_{test}}{n_{total}}$ or equivalent $\frac{1}{k}$
...
We first need to evaluate both models using 10x 10-fold CV splits.  Important that these are the SAME splits for both model configurations
```{r u9-compare-16}
set.seed(2468)
splits_cv <-
  data_all %>% 
...
Here is k-fold accuracy for the full model

```{r u9-compare_full-17}
...
# I will simplify to a reprex and submit a bug report
cv_full <- 
  logistic_reg(penalty = 0.2132798, 
               mixture = 0.2) %>% 
  set_engine("glmnet") %>% 
...
Here is k-fold accuracy for the compact model
```{r u9-compare-18}
cv_compact <- 
  logistic_reg(penalty = 0.2606748,
               mixture = 0.2) %>% 
  set_engine("glmnet") %>% 
...
Calculate the t-test.  In this instance we likely want a one-tailed test (though of course, that should have been planned in advanced and ideally pre-registered!).   My function returns a two-tailed p-value so we should cut it in half.
```{r u9-compare-20}
...
The improvement in prediction accuracy associated with the use of our exercise test protocol is significant (one-tailted).
...
Lets do this step by step.  We will use the `tidyposterior` package that we previously loaded.  
...
* join these dataframes, matching on repeat and fold ids
...
* The mean increase in accuracy is 4.14%
* The 95% HDI is 3.01% - 5.53%
...
```{r pp_contrast_summary}
pp %>% contrast_models(seed = 4) %>% summary(size = .01)
```

Alternatively, using the approach proposed by Kruschke (2018), you can conclude that the full model is meaningfully better than the compact model if the 95% HDI is fully above the ROPE.  In this instance it is (.031 - .0553 is all above .01)
...
For our purposes, the growing set of tools to interpret our models can help address our **explanatory questions**.   But they can also help us **find errors** in our models and **detect possible bias** (we will focus explicitly on algorithmic bias in later units)
...
For some models, interpretation and identification of important features is easy.  For example, if we standardize the variables in a glm or glmnet model, we can interpret the absolute magnitude of the parameter estimates (i.e., the coefficients) as an index of the global (i.e., across all observations) importance of each feature.  

But for other models, we need different approaches.  There are many approaches to quantify the importance of a feature, but we will focus on two that are model agnostic (i.e., can be used across all statistical algorithms)
...
We will use the `iml` package that was released along with @IML to calculate these variable importance metrics.

You can get more help on the use the functions in `iml` in this [vignette](https://cran.r-project.org/web/packages/iml/vignettes/intro.html)

***

Lets first get some coding issues accomplished before we dig into the details of the two feature importance metrics

To calculate these metrics, the `iml` functions will need access to the raw features and outcome. Lets get those again.

```{r get_data}
feat <- rec_full %>% 
  make_features(data_all, data_all, FALSE)

x <- feat %>% select(-disease)  # x features
y <- feat %>% select(disease) # outcome
```

***

We will also need a predictor function to allow the `iml` functions to make predictions using the model.  
...
By randomly shuffling the values for the feature, we break the relationship between that feature and the outcome so it no longer contributes to the predictions.   If performance doesn't change much, then that feature is not important.  If performance goes down a lot, the feature is important.  We can see this relative change in performance by calculating the ratio of the permuted performance to the original performance ($\frac{permuted}{original}$) Simple!   

***

`FeatureImp$new()` supports many performance metrics that can be passed into `loss =` as a string (e.g., `loss = "rmse"`).  See help for a full list.

`iml` doesn't directly support accuracy because it prefers "error" metrics.  As such, is supports "ce" which stands for classification error (1 - accuracy).   

However, you can also pass in a function object instead if you need an unsupported performance metrix.   For example, you could use `loss = Metrics::accuracy` to use the accuracy function from the `Metrics` package.  See help for more details on the use of custom functions.

I am OK with the shift to classification error as it is intuitive to talk about how much error increases when features are permuted

Lets calculated these permuted feature importance values for each feature now.
```{r u9-permute_rmse}
imp_permute <- xfun::cache_rds({
  
  iml::FeatureImp$new(predictor, loss = "ce")

},
rerun = FALSE,
dir = "cache/",
file = "u9-permute_rmse.rds")
```

***

We can use the built in plot function to display the prediction accuracy (with a 95% confidence interval) when each feature is permuted.  We see that when `ca` is permuted, classification error increases by a factor of approximately 1.3.

The next most important feature is from our exercise test: `exer_ang_yes`

```{r}
imp_permute %>% plot
```

**PROGRAMMING NOTE**:  If you wanted to have more control over the plot, you can pipe the results directly into ggplot using `imp_permuate$results %>% `
...
And we can get Shapley values using `Shapley$new()`
```{r local_shapley}
sv <- iml::Shapley$new(predictor, x.interest = x1)
```

There is a standard plot method but its not pretty and for binary classification, the plot of both levels is redundant
```{r}

sv %>% plot
```

I prefer to take control directly with the raw output and ggplot.  The raw output is in `$results`
...
And here is a start to a nicer plot

* This participant's value for `thal_normal` increases the probability of disease by ~ .05 from the mean predicted disease probability.
* This participant's value for `thal_reverseabledefect` decreases their probability of disease by ~ .05 from the mean predicted disease probability.
...
get_shapley_values <- function(df1, predictor){
  iml::Shapley$new(predictor, x.interest = df1)$results %>%
    ...
    dplyr::mutate(feature_value = as.numeric(feature_value)) %>%
    dplyr::select(feature, feature_value, phi)
}
```

And then we can map this function over observations to get the Shapley values for each observation.   
```{r u9-global_shapley}
local_shaps <- xfun::cache_rds({
  
  x %>%
    dplyr::mutate(id = row_number()) %>%
    nest(.by = id, .key = "dfs") %>%   # nest a dataframe for each observation
    dplyr::mutate(shapleys = purrr::map(dfs, \(df1) get_shapley_values(df1,
                                                                       predictor))) %>%
    dplyr::select(-dfs) %>%
    unnest(shapleys) 
...
rerun = FALSE,
dir = "cache/",
file = "u9-global_shapley.rds")

local_shaps %>% head
```

**Programming note**:  This code demonstrates another nice R programming technique using `nest()` and `unnest()` in combination with `map()` and list-columns. For more info, see [this chapter](https://r4ds.had.co.nz/many-models.html) in @RDS2023 and the vignette on nesting (`vignette("nest")`).
...
```{r}
local_shaps %>%
  dplyr::mutate(phi = abs(phi)) %>%
  group_by(feature) %>%
  dplyr::summarize(mean_shapley = mean(phi)) %>%
  dplyr::arrange(desc(mean_shapley)) %>%
  dplyr::mutate(feature = factor(feature),
         feature = forcats::fct_reorder(feature, mean_shapley)) %>%
  ggplot(aes(x = feature, y = mean_shapley)) +
  geom_point() +
  coord_flip()
```

For a more advanced plot (not displayed here) we could superimpose the individual local Shapley values and color them based on the feature score.   This would allow us to show the direction of the relationship between the Shapley values and feature values.  See FIGURE 9.26 in @IML for an example of this type of plot.
...
There are computational shortcuts available but even those approximately can be very time consuming.
...
We can also learn about how our features are used to make predictions in our models using visual approaches.   There are two key plots that we can use:
...
We will use the `iml`  package that accompanies @IML to make PD and ALE plots.

The code is very straight-forward.  Here we get the predicted values for an ALE plot to examine the effect of one of the features from our exercise test (`exer_max_hr`) on disease probabilities. 

If we wanted a PD plot, we could simply substitute `pdp` for `ale`

```{r pdp_plots}
ale <- iml::FeatureEffect$new(predictor,
                  feature = "exer_max_hr",
                  method = "ale",  # or pdp
                  grid.size = 30)
```
...
There is a default plot function that can be used.  It combines the ale/pd plot with a rug plot to show the distribution of values for the target feature.  However, by default, it displays separate plots for all levels of a categorical outcome.   This makes sense if your categorical outcome has three or more levels but not for a binary outcome
...
With two levels for the outcome, it is redundant.  In this instance, you can make your own plot.  You might also do this to gain more control over the plot.

We need a dataframe for the raw values of the target feature for the rug plot.  We also need values for y for the rug plot but I dont plan to show them so I set those to constant (0).

```{r}
# for the rug
feature_values <- x %>% dplyr::select(exer_max_hr) %>% 
...
ale$results %>% 
  filter(.class == "yes") %>% 
  ggplot() +
  geom_line(mapping = aes(x = exer_max_hr, y = .value)) + 
  geom_rug(mapping = aes(x = exer_max_hr, y = .value),
           data = feature_values,
           sides = "b", position = "jitter", alpha = 0.5) +
