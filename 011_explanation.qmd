...
Posterior probabilities are a way for us to estimate the likelihood of a parameter being a certain value. For example, the probability that are model is performing within a certain range.    
...
Shapley values give us a measure of relative feature importance (i.e., which features are contributing most to a model's predictions compared to other features).      
...
*Could we go over again why calculated differences in predictions for ALE plots makes them better suited for handling correlated features than M-plots?*