...
* [PSY 752 course syllabus](https://dionysus.psych.wisc.edu/iaml/pdfs/syllabus.pdf)
* @ISL [Chapter 2, pp 15 - 42](https://dionysus.psych.wisc.edu/iaml/pdfs/ISLRv2.pdf)
* @Yarkoni2017 [paper](https://dionysus.psych.wisc.edu/iaml/pdfs/yarkoni_2017_choosing_prediction_over_explanation.pdf)


**Lecture & Discussion Videos**

* [Lecture 1: Overview](https://www.loom.com/share/c61baac1b7b94ca7bb5c27feb9f99c2a); ~ 40 mins
* [Lecture 2: Terminology](https://www.loom.com/share/aa50f305a79b44aa84c823eeb3dd1fdc); ~ 19 mins
* [Lecture 3: An Empirical Example](https://www.loom.com/share/07b90a3321c84854a96a8eaefed6b6a9); ~ 28 mins
* [Discussion](https://dionysus.psych.wisc.edu/iaml/videos/discussion_unit1_2023.mp4)


Post questions/discuss readings or lectures in Slack

The [unit quiz](https://canvas.wisc.edu/courses/345486/quizzes/424938) is due by 5 pm on Wednesday January 25th
...
```{r u1-gen-1, fig.align = "center", out.height = figsizer(500), echo = FALSE} 
knitr::include_graphics("figs/unit1_data_dgp.png")
```
...
```{r  u1-gen-2, fig.align = "center", out.height = figsizer(500), echo = FALSE} 
knitr::include_graphics("figs/unit1_flex_interpret.png")
```
...
1. Models are **biased** if they don't adequately represent the true `f`
...
## An Example of Overfitting and the Bias-Variance Trade-off 
...
`r colorize("If the DGP for y is a cubic function of x, what do we know about the expected bias for our three candidate model configurations in this example?", "red")`
^[The simple linear model will be biased b/c it can only represent y as a linear function of x.  The two polynomial models will be generally unbiased b/c they have x represented with 20th order polynomials.  LASSO will be slightly biased due to regularization but more on that in a later unit]

```{r u1-ex1-1, echo = FALSE}
...
plot_data <- function(data, line_names, line_colors, line_types, line_labels, title){
  ...
  preds <- data %>% 
    ...
    pivot_longer(cols = all_of(line_names),  
                 names_to = "model", 
                 values_to = "pred") %>% 
    mutate(model = factor(model, levels = line_names, labels = line_labels, ordered = TRUE)) # to maintain order of lines
  
  p <- ggplot(data = data, aes(x = x)) +
    geom_point(data = xy, aes(x = x, y = y), color = "gray") +
    geom_line(data = preds, aes(x = x, y = pred, group = model, color = model, linetype = model), size = 1.25) +
...
2. We get one more large random sample (N = 5000) with the same DGP to use as a test set to evaluate all the models 
that will be fit in the training sample across all of the simulations.

```{r u1-ex1-11, echo = FALSE}
params$n = 5000 
...
`r colorize("Look in any training set.  Can you see evidence of bias for any model configuration?", "red")`
^[The simple linear model is clearly biased.  It systemically underestimates Y in some portions of 
the X distribution and overestimates Y in other portions of the X distribution.   
This is true across training sets.]

`r colorize("Can you see any evidence of overfitting for any model configuration?", "red")`
^[The polynomial linear model appears to overfit the data in the training set.  In other words,
it seems to follow both the signal/DGP and the noise.  However, you can't be certain
of this with only one training set and without knowing the DGP.  
...
`r colorize("Can you see evidence of bias for the simple linear models?", "red")` 
^[Yes, consistent with what we saw in the training sets, the simple linear model systematically
overestimates Y in some places and underestimates it in others.  The DGP is clearly NOT linear
but this simple model can only make linear predictions.  It is a fairly biased model.  
This bias will make a large contribution to the reducible error of the model]

`r colorize("How much variance across the simple linear models is present?", "red")` 
^[There is not much variance in the prediction lines across the models that were fit to different
training sets.  The slopes are very close across models and the intercepts only vary by 
a small amount.   The simple linear model configuration does not appear to have high variance
and model variance will not contribute much to its reducible error.]
...
`r colorize("Are they systematically biased?", "red")`
^[There is not much systematic bias.  The overall function is generally cubic as is the DGP.  
Bias will not contribute much to the model's reducible error.]

`r colorize("How does the variance of these models compare to the variance of the simple linear models?", "red")`
^[There is much higher model variance for this polynomial linear model relative to the simple linear model. Although
all four models generally predict Y as a cubic function of X, there is also a non-systematic wiggle that is 
different for each of the models.]

`r colorize("How does this demonstrate the connection between model overfitting and model variance?", "red")`
^[Model variance is a result of overfitting to the training set.  If a model fits noise in its training set,
that noise will be different in every dataset.  Therefore, you end up with different models depending on the
training set in which they are fit.  And none of those models will do well with new data as you can see in this
...
`r colorize("How does their bias compare to the simple and polynomial linear models?", "red")`
^[It has low bias much like the polynomial linear model.  It is able to capture the true cubic DGP
fairly well.  The regularization process slightly reduced the magnitude of the cubic
(the prediction line is a little straighter than it should be) but not by much.]


`r colorize("How does their variance compare to the simple and polynomial linear models?", "red")`
^[It has low variance, much like the simple linear model.  All four models, fit in different
training sets resulted in very similar prediction lines.]
...
`r colorize("What do we expect about RMSE for the three models in train and test?", "red")`
...
`r colorize("Would these observations about bias and variance of these three model configurations always be the same regardless of the DGP?", "red")`
...
2. Structure of course
    * Same flow each week
      ...
      * Quiz and application assignments due Wednesdays at 5 pm
...
7. Environment
    ...
    * Have fun
    * [Accomodations](http://dionysus.psych.wisc.edu/iaml/index.html#accommodations-polices) and [Complaints](http://dionysus.psych.wisc.edu/iaml/index.html#complaints)
...
* `r colorize("What is the difference between association vs. prediction?", "red")`
...
* `r colorize("Describe problem of p-hacking with respect to overfitting?", "red")`
...
```{r  u1-diss-1, fig.align = "left", echo = FALSE}
# , out.height = figsizer(fh5)
knitr::include_graphics("figs/unit1_dartboard.png")
```
...
`r colorize("Are GLMs biased models?", "red")`
...
`r colorize("Explain the link between model variance/overfitting, standard errors, and sampling distributions?", "red")`
