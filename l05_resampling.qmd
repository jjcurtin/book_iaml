...
# IAML Unit 5: Resampling Methods for Model Selection and Evaluation
:::
...
::: {.fragment .uwred}
Increasing the sample size used to fit our model configuration will decrease model variance but not change model bias. This will produce overall lower 
error.

This might not be true if the additional data were not similar quality to our training data but we know our validation/test set is similar because we did a
random resample.
:::
...
when estimating the performance of our final model configuration (but this will come  with other costs!)
:::
...
Our best estimate of how accurate a model with k = `r select_best(fits_knn_boot_trn, metric = "accuracy")$neighbors` would be in new data is `r accuracy_vec(feat_test$disease, predict(fit_knn_best, feat_test)$.pred_class)`.   
