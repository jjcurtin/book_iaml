...
* One RMarkdown (.Rmd) file: hw_unit_11_neural_nets.Rmd (this file)

* wine_quality_trn.csv & wine_quality_test.csv (training & test data for this assignment)

The data for this week's assignment include wine quality ratings for various white wines. The outcome is the "quality" variable (categorical: "high_quality" or "low_quality"), and there are 11 numeric predictors that describe attributes of the wine (e.g., acidity) that may relate to the overall quality rating. 

We will be doing another competition this week. You will fit a series of neural network model configurations and select the best configuration among them. You will use the wine_quality_test.csv file **only once** at the end of the assignment to generate your best model's predictions in the held-out test set (the test set will not include outcome labels). We will assess everyone's predictions on the held-out set to determine the best fitting model in the class. **The winner again gets a high five from John & a free lunch (or equivalent)!**
...
Your assignment is due next Wednesday at 8:00 PM. Let's get started!
...
Change the path to be where your Unit 11 homework files are saved.
```{r}
path_hw_unit_11 <- ""
```
...
Source the functions script/s that you need to do your modeling for the assignment.
...
Strongly recommended! You can use the code that youve used for previous assignments here.
```{r}
...
## Save & knit
Save this .Rmd file with your last name at the end (e.g., hw_unit_11_neural_nets_name.Rmd). Make sure you changed "Your name here" at the top of the file to be your own name. Knit the file to html. Upload the knit file and your saved test_preds_lastname.csv to Canvas.  
