...
**Two RMarkdown files (hw_unit_2_cleaning.Rmd and hw_unit_2_modeling.Rmd):** 
To complete this homework (and the other homeworks) type all responses directly into these Rmarkdown files. To learn more about working in Rmarkdown files, see the R Markdown chapter of R for Data Science.
...
Some code chunks, like the `packages` chunk below, will be pre-filled with code for you to run. Other code chunks will be empty with text instructions above them describing code for you to fill in.
...
You may also use **fun_modeling.R:**
A script containing functions written by John that are demoed in the course text.  This script is
available on Github in a repository that John shares.  You can download the current version directly from GitHub  using the code below (note that you may need to install the `devtools` package if you haven't done this previously).
...
In R Markdown, when  you are talking about variables, R functions, etc., you can display that text distinctly (with a grey background) when the script is knit by surrounding the text with `.  We will use this in the web book and our homework scripts 

Note that other mathematical elements in RMarkdown are by convention surrounded by `$` when specifying them inline. This renders the enclosed element in a distinct typeset when knit. You'll learn more about this notation later, but it is especially useful when composing mathematical elements (like equations) in your notebooks. 

For example: 2^2 + 4 = 8$

Make sure you have your iaml project open in RStudio.  When you run `library(here)` it will set your root path to be inside of the iaml folder.   Assuming you have a subfolder called homework and a folder within that folder called `unit_2`, `path_data` will work as set.  If you have some other organization, you will need to modify `path_data` to reflect that folder structure.
...
Confirm that all variables are read in as the expected class. Below the code chunk, type some observations you have about the observed variable classes compared to descriptions in the data dictionary. Make any appropriate adjustments to variable class below using `mutate()`, `as.character()`, or `as.numeric()`.
```{r clean-class}
...
### 4.1 Save
Save this `.Rmd` file with your last name at the end (e.g. `hw_unit_2_Santana.Rmd`)

### 4.2 Knit
Click the "Knit" button next to the blue yarn ball (with needle) at the top of your screen to knit the file to html. Upload this knit html file to Canvas to submit the first part of the assignment. Take care not to rest on your laurels: you have a 2nd part to do (but we are proud of your for getting this far!).