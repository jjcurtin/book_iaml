...
Set all variables to factor or numeric classes. Here is where you will also want to explicitly set factor levels for those with low frequency count levels (e.g., `neighbohood`, `ms_sub_class`)
...
Use `rmse_vec()` to get error in data_trn
```{r}

```

Use `rmse_vec()` to get error in data_test
```{r}
