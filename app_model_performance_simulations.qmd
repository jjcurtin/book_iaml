...
* and we have `r n_features` features

Lets fit many knn models of n = `r n_obs` (the size of our final model) and assess performance in really big samples of new data (ah the luxury of simulated data!) to get a "TRUE" estimate of how well our model trained with full N performs in a low variance (very big) sample of new data.
```{r}
...
message("True RMSE = ", round(rmse_true, 3))
```
