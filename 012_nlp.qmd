...
```{r, u12-imdb-5}
data_trn <- read_csv(file.path(path_data, "imdb_trn.csv")) %>% 
...
Given that **tokenization** is often one of our first steps for extracting features from text, 
it is important to consider carefully what happens during this step and its implications for your subsequent modeling
...
```{r u12-tokens-1}
"Here is a sample document to tokenize.  How EXCITING (I _love_ it).  Sarah's has spent 4 or 4.1 or 4P or 4+ or >4 years developing her pre-processing and NLP skills.  You can learn more about tokenization here: https://smltar.com/tokenization.html or by emailing me at jjcurtin@wisc.edu" %>% 
...
- `str_replace(word, ".$", "")`
```{r u12-tokens-3}
"Here is a sample document to tokenize.  How EXCITING (I _love_ it).  Sarah's has spent 4 or 4.1 or 4P or 4+ years developing her pre-processing and NLP skills.  You can learn more about tokenization here: https://smltar.com/tokenization.html or by emailing me at jjcurtin@wisc.edu" %>% 
...
```{r u12-tokens-20}
data_trn_cln <- # could return into data_trn instead
  tokens %>%  
...
Subject-specific stop words are words that are common and uninformative given the subject or context within which your text/documents were collected and your modeling goals.  For example, given our goal to classify movie reviews as positive or negative, subject-specific stop words might include:
...
For example, do we need to distinguish between movie vs. movies or actor vs. actors or should we collapse
those pairs into a single token?

-----

There are many different algorithms that can stem words for us (i.e., collapse multiple
versions into the same base).  However, we will focus on only one here as an introduction to the concept
and approach for stemming.
...
- We are timing it to demonstrate that this is a fast procedure
...
Let's compare vocabulary size.  Stemming produced a sizable reduction in vocabulary size
...
In this example, we focused on unigrams.  If we had wanted bigrams, we would have needed a different order of steps
...
Instead it is the raw count divided by the document length.  This removes the bias based on document length

```{r u12-bag-3, echo=FALSE}
tibble::tribble(
  ~Document,                           ~i, ~loved, ~that, ~movie, ~am, ~so, ~happy, ~was, ~not, ~good,
  "I loved that movie! I am so so so happy.", .2, .1, .1, .1, .1, .3, .1, 0, 0, 0,
...
We will start by fitting BoW model configurations for word and n-gram tokens

All of these models will be tuned on `penalty`, `dials::mixture` and `max_tokens`
```{r u12-ex-3}
...
- Recipe for Stemmed Word Tokens.  NOTE:
  - `step_stem()`
```{r u12-ex-14}
...
fits_stemmed_word <- xfun::cache_rds({
...
BoW is an introductory approach for feature engineering.  As you have read, word embeddings 
are a common alternative that addresses some of the limitations of BoW.  Word embeddings 
are also well-support in the `tidyrecipes` package.
...
I previously downloaded pre-trained GloVe embeddings

- Using `fread()` because the file is big
...
```{r u12-ex-23}
# library(data.table)
# requires R.utils to be installed to use fread with .gz file
# glove_embeddings <- fread(file.path(path_data, "glove_6B_100d.csv.gz")) %>%
#   as_tibble() %>% 
#   glimpse()
```
...
rec_final <-
  recipe(sentiment ~ text, data = data_trn) %>%
    step_tokenize(text, 
                  engine = "tokenizers", token = "ngrams",   # included for clarity
                  options = list(n = 2, n_min = 1)) %>%
...
- Variable importance scores
   ...
   - set up predictor function and impute
   - these computations would take a while given the number of observations (25,000) and the number of features (10,000).  To speed this up, we use the native predict function for a glmnet object.  I also take a random sample of observations and only do 1 permutation per feature (that should be increased for real use)

```{r u12-ex-31}
# random sub-sample of training data
set.seed(12345)
feat_sample <- feat_trn %>% 
  slice_sample(prop = .05) # 5% of data (1250 observations rather than 25000)

# x and y from features
x <- feat_sample %>% select(-sentiment)  # x features
y <- feat_sample %>% pull(sentiment) # outcome

# Could have used this prediction function (commented out below) with the full parsnip object but that object is 500 MB and very slow to predict with.  Instead, we demonstrate pulling out the fit object from the full parsnip model and predicting using the native predict function for a glmnet model.  

# predictor <- function(model, x){
#     predict(model, x)$.pred_class # using predict.parsnip
# }

# predictor function using predict.glmnet
predictor <- function(model, x){
    predict(model, newx = as.matrix(x), type = "class", s = 0.2069138) %>% 
    factor(levels = c("neg", "pos"))
}

# name of variables to compute variable importance for
var_strings <- names(x)

imp_perm <- var_strings %>% 
  map(\(var_string) get_vip(
                          # use only the glmnet fit rather that the full
                          # parsnip model.  Its smaller
                          model = fit_final$fit, 
                          x = x, y = y,
                          var_string = var_string,
                          fun_pred = predictor, 
                          # can use any `_vec` metric in yardstick
                          fun_metric = yardstick::accuracy_vec,  
                          # set n_reps higher (5, 20?) for real applications
                          n_reps = 1)) %>%   
  list_rbind()
```

- Full pipe to plot top 30 in an informative display
```{r u12-ex-32}
imp_perm %>%
  ...
  slice_tail(n =30) %>% 
  ggplot(mapping = aes(x = variable, y = metric_mean)) +
  geom_point(size = 2, color = "red") +
  geom_segment(aes(x = variable, y = metric_mean, xend = variable), 
               yend = 0, colour = "grey50")  +
  ylab("Reduction in Accuracy") +
  coord_flip()
```
...
```{r u12-ex-33}
data_test <- read_csv(file.path(path_data, "imdb_test.csv")) %>% 
  rowid_to_column(var = "doc_num") %>% 
  mutate(sentiment = factor(sentiment, levels = c("neg", "pos"))) %>% 
  glimpse()
