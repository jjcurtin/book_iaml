...
Its a bit more complicated for Windows (isn't it always?).  In a fresh environment (immediately after starting RStudio), you need to:
...
Contact us immediately if you have trouble configuring Keras so we can help you troubleshoot the installation before you need it for the application assignment.  


## Setting up our Environment 


Now lets start fresh.  We have just opened RStudio.  

We load our (mostly) normal packages

We do NOT need to load Keras


Some new things:

* We will use `data.table` for reading and writing large datafiles

* We will use `imager` for viewing images saved as matrices
 
```{r u11-env-1, warning=FALSE, message=FALSE}
library(data.table) #for fread(); fwrite()
library(imager) # for as.cimg(); plot.cimg()
library(tidyverse)
library(tidymodels)
...
```{r u11-mnist-1}
data_trn <- read_csv(here(path_data, "mnist_train.csv.gz")) %>% 
  mutate(y = factor(y, levels = 0:9, labels = 0:9))
data_trn %>% dim()

data_test <- fread(here(path_data, "mnist_test.csv")) %>% 
    mutate(y = factor(y, levels = 0:9, labels = 0:9))
data_test %>% dim()
```
...
Let's look at some of the images.  We will need a function to display these images.  We will use `as.cimg()` and `plot_cimg()` from the `imager` package.
```{r u11-mnist-4}
