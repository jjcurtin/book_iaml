...
- You need to list the path (`dir =`) and filename (`file =`) for the rds file that will save the cached calculations.  The `/` at the end of the path is needed.  You should use a meaningful (and distinct) filename.
...
::: {.callout-important collapse="false"}
### Question: But what does this mean about our estimate of the performance of this final model using all data when we get that estimate using a smaller sample size (the sample size in our training set)?
```{html}
#| echo: true
#| code-fold: true
#| code-summary: "Show Answer"
...
sample size of the training set and the size the of full dataset.   If we want less biased estimates, we want to allocate as much data as possible to the training set when estimating the performance of our final model configuration (but this will come with other costs!)
```
:::
...
- If you change the raw data file but don't update rec or splits, it will not. Be careful! 
