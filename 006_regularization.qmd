...
- [qmd shell](application_assignments/unit_06/hw_unit_6_regularization.qmd)
- [solution]() 
...
Set up a tibble to track model performance in train, val, and test sets

- We are using test to repeatedly to get rigorous held-out performance seperate from model selection process.  
...
```{r}
grid_penalty_lasso <- expand_grid(penalty = exp(seq(-4, 4, length.out = 500)))
```
...
fits_lasso_1 <- xfun::cache_rds(
  ...
  linear_reg(penalty = tune(), 
    ...
    tune_grid(preprocessor = rec_1, 
              resamples = splits_boot_1, grid = grid_penalty_lasso, 
...
```{r}
grid_penalty_ridge <- expand_grid(penalty = exp(seq(-1, 7, length.out = 500)))
...
fits_ridge_1 <- xfun::cache_rds(
  expr = {
    ...
    set_engine("glmnet") |> 
    tune_grid(preprocessor = rec_1, 
              resamples = splits_boot_1, grid = grid_penalty_ridge, 
              metrics = metric_set(rmse))
...
fits_glmnet_1 <- xfun::cache_rds(
  expr = {
    ...
    set_engine("glmnet") |> 
    tune_grid(preprocessor = rec_1, 
              resamples = splits_boot_1, grid = grid_glmnet, 
              metrics = metric_set(rmse))
  ...
  bind_rows(tibble(model = "Ridge model",                       
                   rmse_trn = rmse_vec(truth = feat_trn$y, 
                                       estimate = predict(fit_ridge,
                                                          feat_trn)$.pred),
                   rmse_test = rmse_vec(truth = feat_test$y, 
                                        estimate = predict(fit_ridge,
                                                           feat_test)$.pred))))
```
