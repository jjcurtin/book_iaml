...
data_all <- read_csv(here::here(path_data, "ames_raw_class.csv"),   # <1>
                     col_types = cols()) |> # <2> 
  mutate(across(where(is.character), as_factor)) |> # <3> 
  glimpse() # <4>
```
...
3. Then we use a mutate to re-class all character data to factors.  This should handle most of our nominal and ordinal variables.  We may need to still class some that were missed because they appeared to be numeric.
...
```{r u2-clean-3}
oq_levels <- as.character(1:10) # <1>

data_all <-  data_all |> 
  mutate(overall_qual = fct(as.character(overall_qual), # <2>
                            levels = oq_levels)) |> # <3>
  glimpse()
```
1. It is always best to explicitly set the levels of an ordinal factor in the order you prefer.  You may also want to explicitly set the levels for a nominal factor as well for error checking purposes (see below).  The levels should be provided as character class.
2. We prefer to use `forcats::fct()` for creating factors.  It is stricter than the base R equivalent (`factor()`).  `fct()` requires that the variable be character class prior to conversion.  If numeric, just change to character in place.
3. We indicate the levels here.  `fct()` will then generate an error if any value is not a valid level
...
The final task we typically do as part of the data preparation process is to split
the full dataset into training, validation and test sets.
...
```{r}
 data_trn <- 
  read_csv(here::here(path_data, "ames_clean_class_trn.csv"), 
           col_types = list(sale_price = col_double(), # <1>
                            gr_liv_area = col_double(),
                            lot_area = col_double(),
                            year_built = col_double(),
                            overall_qual = col_factor(levels = as.character(1:10), # <2>
                                                      ordered = TRUE),
                            garage_cars = col_double(),
                            garage_qual = 
                              col_factor(levels = c("no_garage", "po", "fa", "ta", "gd", "ex"), 
                                         ordered = TRUE),
                            ms_zoning = col_factor(), # <3>
                            lot_config = col_factor(),
                            bldg_type = col_factor())) |> 
  glimpse()
```
1. Use `col_double()` to class numeric columns
2. For ordered factors, use `col_factor()` and need to indicate `levels` in order and set `ordered = TRUE`
3. For unordered factors, use `col_factor()` again but can just use the default ordering unless there is a reason to re-order

* Alternatively, you can read the data using `col_types = cols()` and then use mutate statements to class the columns as needed.  Both approaches follow best practices.
-----
...
```{r u2-recipes-1}
 data_trn <- read_csv(file.path(path_data, "ames_clean_class_trn.csv"), 
                      col_types = cols()) |> 
  mutate(across(where(is.character), as_factor), # <1>
         overall_qual = ordered(overall_qual, # <2>
                               levels = as.character(1:10))) |> 
  glimpse()
```
1. We first use `as_factor()` to set up all character variables as unordered since most are unordered
2. We then make `overall_qual` ordred using the `ordered()` function.
...
* You will bake the train, validatation, and test sets before fitting or evaluating models for now (some of this is automated in the future)
  ...
  * THIS IS HOW WE PREVENT DATA LEAKAGE FROM TRAINING INTO INTO VALIDATION OR TEST SETS!
-----
