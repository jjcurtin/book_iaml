...
* In some cases, we should pay some attention to the order of the levels of the variable
...
* We store (in csv files) these variables as character class with descriptive text labels for the levels
	* Easier to share/document
	* Reduces errors
* We class these variables in R as factors
  * It is easier to do EDA with these variables classes as factors
  * We use standsrd factors (not ordered)
* Confirm that the order of the levels is set up correctly
* During feature engineering stage, we can then either
  * Treat as a nominal variable and create features using `step_dummy()`
  * Treat as a interval variable using `step_ordinalscore()`
...
3. Then we use a mutate to re-class all character data to unordered factors.
...
Here is an easy way to convert responses for character variables to snake case using a function (`tidy_responses()`) we share in `fun_eda.R` (reproduced here). This uses regular expressions (regex), which will will learn about in a later unit on text processing.
