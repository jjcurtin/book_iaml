...
- we haven't considered learning rate yet but that matters too as discussed in the book/videos so lets tune it
```{r}
grid_mlp <- expand_grid(hidden_units = c(5, 10, 20, 30, 50, 100), 
                          learn_rate = c(.001, .01, .1, 1))
```
...
```{r}
plot_hyperparameters(fits_nn, hp1 = "hidden_units", hp2 = "learn_rate", metric = "accuracy")
```

Looking at this hyperparameter plot, we might want to consider even higher number of hidden units and a higher learning rate!
