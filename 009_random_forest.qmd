...
- [qmd shell](homework/unit_10/hw_unit_10_random_forest.qmd)
- [solution]()
...
Figure 8.4 shows a slightly more complicated decision tree for the hitters dataset with tree depth = 4
\
\
...
- The algorithm first identifies the "best" feature to partition the observations in the root node into one of two new regions (i.e., new nodes that will be on the left and right branches leading from the root node.)
  ...
  - For classification problems, the split is selected to maximize the reduction in cross-entropy or the [Gini index](https://enhttps://en.wikipedia.org/wiki/Decision_tree_learning#Information_gain.wikipedia.org/wiki/Decision_tree_learning#Gini_impurity).  These are measures of impurity (and we want to get homogeneous nodes so we minimize them)
...
- We let the tree grow large (max depth = 30 on 32-bit machines) and then prune it back to an optimal size:
...
Feature engineering for decision trees can be simpler than with other algorithms
\
\
There are few pre-processing requirements
...
- But how well does this model predict `sale_price`?
```{r}
...
The best values for all hyperparameters are not near their edges and we see floor in RMSE so this looks good
```{r}
show_best(fits_tree)
```

This is as good as our other models in unit 3 (see OOB cross-validated error above).  It was easy to fit with all the predictors.  However, we can still do better.
...
Even though decision trees themselves are relatively poor at prediction, these ideas will be key when we consider ensemble approaches.   
\
\
...
\
\
Now we need `times = 20` to fit 20 models to 10 bootstraps (set earlier) of the training data.  
...
Random forests builds on decision trees (its base learners) and bagging to reduce final model variance.
\
\
...
1. Explain how decision tree algorithm works generally

2. How does CART and RF handle categorical variables?  What about other transformations of predictors (normalize, scale, combine)? [non-linear?](https://bradleyboehmke.github.io/HOML/DT.html#partitioning)

3. How do you do bagging for decision trees?  How do you select best bagged tree?

4. When and Why does bagging improve model performance

5. Why does bagging not work optimally and how does random forest improve on it?
...
#### Interpretability
- Can you explain how boosting decision trees are more interpretable than random forest? I understand that it builds on top each model so that it slowly grows over time reducing the residuals from the models, but I'm still confused on it's interpretability.

- Could we please discuss the interpretability of parametric vs machine learning models vs decision tree models. It seems that machine learning and smaller decision tree models are relatively easy to interpret, yet in a lot of papers the interpretability of such models is viewed as a negative in comparison to parametric models. 

- Are able to determine the directionality of how the features are impacting the outcome)

#### Feature engineering
- Why do decision tree models not require feature engineering of nominal variables, whereas random forest models do?

- how will decision tree handle missing values in features? 

- I am very interested in and still a little bit confused about how random forest and decision trees can run with such minimal feature engineering. I would like to hear a little bit more about how these methods differ in comparison to previous methods we've used, which have required much more feature engineering

#### Bagging

- How does bagging affect the bias specifically?

- I am interested in understanding better how techniques like bagging can reduce variance so drastically without seeming to affect bias much.

- Bagging and random forests have no impact on model bias, correct?

- Is it correct that we will not use bagged trees in any situations? 

- I am still confuse of difference between using bootstrap on decision tree and bagging

#### Bootstrap

- What is the difference of bagging and normal bootstraps?
- What are the "outer loop" and "inner loop" of bagging?

#### Random Forest

- What are the differences between decision trees and random forests? When do you want to use a decision tree versus a random forest?

- Could you explain a method of Variable Selection for Random Forest?

- Is the logic for preferring random forests over bagging essentially the same as the logic for preferring k-fold cross-validation with k < n over LOOCV?

- Can you talk more about real world use in random forest? 

#### Boosting

- Boosting a decision tree
- boosting versus other ensemble techniques like random forest
