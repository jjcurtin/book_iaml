...
A quick hyperparameter primer:
 - $\alpha$: called *mixture* in `tidymodels`; represents the mix we want between LASSO and Ridge in Elastic Net/GLMNet (these are held at constant values for LASSO and Ridge, as you can note in the table belows)
 - $\lambda$: called *penalty* in `tidymodels`; represents the strength of our regularization; the combination of $\alpha$ = 1  with a penalty is what allows less features to be dropped in LASSO, whereas the $\alpha$ = 0 in Ridge with a penalty makes less important features instead very small (close to 0)
...
Note that we get an error here because we have some perfectly collinear columns! We have 13 predictor variables, but two are exact duplicates. OLS cannot handle these duplicates so one is dropped, leaving us with 11 final features.
```{r}
...
In Ridge regression, we want to tune $\lamba$ (`penalty`), so we'll create a grid of possible values to test out. Let's take a look at what that looks like. Keep in mind that the `exp()` function means we are sequencing 50 values from $e^{-2}$ to $e^{2}$.
```{r}
...
We want to tune $\lamba$ (`penalty`) again for LASSO, so we'll keep the same tuning grid as above. This time, we'll be setting $\alpha$ (`mixture`) to 1.
```{r}
plan(multisession, workers = parallel::detectCores(logical = FALSE))
