...
JJC: It is OK to consider dropping missing value observations from training set rather than imputing. You can compare that approach to imputation by considering these different model configurations and comparing with held out data.   You can NOT drop missing values in held out data.  You have to be able to make predictions for all held out cases.   There, you will have to impute (with imputation method and values determined from training data.)

It is generally easier to just impute in training too. rather than drop cases
...
JJC, correct as above with added that for ordinal, you can choose between dummy versus step_ordinal (assigning ordered numbers) by treating these as different model configurations and comparing them using held-out (validation) data.
...
JJC: with splits set up this way, you can extract train and test using `analysis()` and `assessment()`
...
JJC:  dont forget about `fct_relevel()`.  That is probably the more appropriate way to do this versus setting up new factor
```{r}
# original plot
...
JJC: use always `prep()` with training/held-in data.   bake features for training using prepped recipe with `new_data = NULL`.  Bake features for held-out (val, test), by passing that dataset into `new_data`
