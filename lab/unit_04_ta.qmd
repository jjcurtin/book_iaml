...
```{r}
rec <- recipe(status ~ seniority + marital + job + amount + income + home, data = data_trn) |>
  step_mutate(
...
```{r}
rec_prep |> tidy(number = 1) # mutate

rec_prep |> tidy(number = 2) # impute_median

rec_prep |> tidy(number = 3) # impute_mode
```
:::
...
fit_lr <- 
  ...
  fit(status ~ ., data = feat_trn) # this is where our model is ACTUALLY fit
...
error_val <- bind_rows(error_val, 
                       tibble(model = "logistic reg", 
                              accuracy = accuracy_vec(feat_val$status,
                                                          predict(fit_lr,
                                                                  feat_val,
...
```{r}
predict(fit_lr,feat_val, type = "class")
```
...
```{r}
predict(fit_lr,feat_val, type = "class")$.pred_class
```
...
```{r}
accuracy_vec(feat_val$status,predict(fit_lr, feat_val, type = "class")$.pred_class)
```
...
error_val <- bind_rows(error_val, # take errror_val, and append the following:
                       tibble(model = "5nn", # what we want in our model column
                              accuracy = accuracy_vec(feat_val$status, # what we want in our accuracy column
                                                          predict(fit_5nn,
                                                                  feat_val,
                                                                  type = "class")$.pred_class)))
error_val <- bind_rows(error_val, 
...
Let's pivot over to looking at our assignment that was due last week.
We'll take a look at the key and walk through how to generate
predictions, because a number of folks had difficulty generating these
files!
