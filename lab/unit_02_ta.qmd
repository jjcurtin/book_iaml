...
This can cause errors if you are wanting the function from the first package and don't use the namespace (i.e., reference the function using the name of the package, à la: `janitor::clean_names()` to explicitly call the `clean_names()` function from the `janitor` package). Even worse it might not generate an error and give you inaccurate results without you realizing it!    
...
Now we'll do a simple example together. In our first error example above, we ran the following code and got an error. We just talked about why this error happens, but let's pretend like we have no idea and create a reproducible example of it.
```{r}
bind_rows(ames, data.frame(sale_price = c("200000", "220000")))
```
