...
3.  Now I tune a grid with many more values of lambda using just the basic `tune_grid()` approach and setting `grid = grid_large `
...
4. Now I explicitly pass in `path_values` into `set engine()` as per this [documentation](https://parsnip.tidymodels.org/reference/glmnet-details.html) but still use a grid with the same values of lambda
...
5. Next I tried using a grid with alpha and only one value of lambda but I still passed in full regularization path with `path_values`
...
<sup>Created on 2026-02-21 with [reprex v2.1.1](https://reprex.tidyverse.org)</sup>